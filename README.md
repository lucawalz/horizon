...
| `spec.hetzner.imageSelector` | deprecated, one of `imageSelector`/`image` required | One or more labels selecting the boot image; the newest match wins. Replaced by `spec.hetzner.image`. |
| `spec.hetzner.image` | one of `imageSelector`/`image` required | Selects the boot image by exactly one of `name`, `id` or `selector`. |
