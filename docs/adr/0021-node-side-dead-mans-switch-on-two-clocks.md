...
The rule is cross-resource, so CEL cannot express it, and there is no admission webhook in the repository to hold it either. The controller therefore derives `status.expiresAt` on every reconcile as the earlier of the requested deadline and the backstop, rather than stamping it once at acceptance, and reports an `ExpiryClamped` condition when the backstop is the one that holds. Clamping is preferred over refusing the lease, because the failure worth eliminating is a silent overrun.

Deriving also closes a leak that predates the clamp and was never claimed. When a status write is lost, acceptance replays with `status.acceptedAt` back at nil while the instances it already created keep billing. The stamped version wrote a fresh `now` plus the full duration, so a lease could be renewed indefinitely, an instance at a time, with nothing anywhere recording that it had already run. The derived deadline is bounded by the surviving instances' own backstops, so a replayed acceptance clamps at once rather than granting another full duration.

The backstop is latched per instance, not recomputed. `status.instances[].backstopAt` is written once, when the instance enters the lease's status, as its creation instant plus the `maxLifetime` in force at that moment, and nothing rewrites it afterwards. The lease's backstop is then the earliest latched value across the instances it still holds. Recomputing from the live `ProviderConfig` instead was tried and is wrong in both directions, both reproduced by test:

- Lowering `maxLifetime` on a config that live leases are bound to drove `status.expiresAt` into the past. Because the teardown budget is anchored at the earlier of the deletion timestamp and the deadline, the whole grace was already spent before teardown started, so no node drained and every workload on every lease bound to that config was killed outright, by one edit, with no lease owner involved. It also billed the corrupted anchor into `horizon_lease_release_seconds`, recording twenty-five minutes for a release that took none.
- Raising it moved the controller's backstop past the machine's. The safety property is that `earliestCreatedAt + maxLifetime` never exceeds `agentStart + maxLifetime`, and it holds only while both sides read the same `maxLifetime`. The machine still destroys itself, because its two clocks are combined with an or, so this leaks no capacity; it falsifies the cluster's belief rather than the guarantee.

Latching restores the invariant, because the value the controller compares against is the one the machine booted with. It is still conservative, and deliberately: the agent measures its lifetime from process start, which the cluster cannot observe, so the latch anchors on the instance's creation instead, which is always earlier.

Absence is reported rather than assumed. An instance that records no `backstopAt`, which is every instance already running when this change is deployed, leaves the lease with no value to check its deadline against. That is not the same as a lease that fits, and reporting the two identically would hide the one state where the safety net is missing, so `ExpiryClamped` reads `Unknown` with reason `BackstopUnknown` while any held instance has nothing latched. Leases in flight at upgrade carry it until their instances are replaced.

One gap is recorded here rather than closed, and latching does not close it. The `horizon.dev/expires-at` label is written once in the create call and the provider seam offers no way to update it, so it goes stale on a lease whose deadline later moves. The controller never reads that label, so the derivation does not depend on it. The agent does: it reads the label at start to seed its wall clock before its first node read, and the node annotation supersedes that seed on the first successful poll, so a stale value governs for at most one poll interval.
