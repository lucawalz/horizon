...
The verb is `Extend(ctx, name, duration)`. It names a lease and the duration that lease should run for. It accepts no object, no patch and no field path, and the JSON merge patch that reaches the apiserver is built inside the implementation, naming `spec.duration` and nothing else. A general `Update` or a general `Patch` would have handed every holder of a writer the ability to move any field of any object the caller's RBAC reaches, which is a far wider capability than the two verbs this record narrowed the seam to, and it would have made the seam stop describing what a process holding it may do. A purpose-built verb adds one capability and states it in the type, so the seam keeps saying what it said.

One consequence is that a bare `client.Client` no longer satisfies `LeaseWriter`. Both serving modes wrap their client in `web.LeaseWriterFor`, which is the only implementation this repository ships, and `internal/impersonate` wraps the per-request impersonated client in the same way. That direction is deliberate: the ability to write is now acquired by an explicit construction rather than carried in by a cluster client that happened to be at hand.
...
The documented user role in `docs/serving-the-interface.md` grants `get, list, watch, create, delete` on `capacityleases` and no `patch`, so an in-cluster interface serving a user bound to it answers the apiserver's own `403` to an extension until that verb is added to the role.
