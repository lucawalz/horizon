...
The lease controller wakes on `Node` events in the reserved pool, filtered by the predicate the orphan reconciler already uses, extended with one signal the orphan reconciler does not take. Three things wake a lease: a readiness change, an adoption-label change, and the arrival or departure of `horizon.dev/watchdog-armed`. Without a filter, every kubelet status update would wake a reconcile that lists nodes uncached and writes status. Without the third signal, the filter would swallow the first arming and leave the `WatchdogArmed` condition from [0023](0023-observe-the-armed-watchdog-from-the-control-plane.md) waiting for a requeue.
