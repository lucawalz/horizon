...
The per-instance stages aggregate to the least advanced one, which picks the `InstancesReady=False` reason and message. The message names the blocking instance and how long it has been waiting, computed from `entry.CreatedAt`, which is the provider's own creation timestamp and is already the input to the fifteen-minute registration timeout. The `%d of %d nodes ready` count is kept wherever it can be observed, and follows the diagnostic rather than leading it, because the web interface truncates a condition message and shows the rest only on hover, and no consumer parses the message for its parts: `internal/web/leases.go` reads the condition's status alone.
...
That earlier report carries the stage but not the count. A count needs a node listing, and the step that runs before instance creation has none; the only figure available to it is the number of entries in `Joined`, which records that a node was ready once rather than that it is ready now. Reporting from the phases produced a message claiming one of two nodes ready when none were. The count is therefore omitted where it cannot be observed, rather than estimated from a field that does not mean what the count means.
...
The published dataset stays as published. It was gathered under the poll and is quantised to thirty seconds, and this change does not retrospectively improve it. The defensible reading is that true time to ready lies in (30, 60] seconds for nineteen of thirty runs and in (60, 90] for eleven, and the instrument cannot say where inside those intervals. The medians and the null result on location are unaffected, because the same grid applied to every run. Any future measurement taken after this change is taken on a different instrument, and comparing the two directly compares instruments as much as machines.
...
The lease controller wakes on `Node` events in the reserved pool, filtered by the predicate the orphan reconciler already uses, extended with one signal the orphan reconciler does not take. The predicate constrains updates and nothing else, so a pooled node registering or disappearing wakes the lease unconditionally, and registration is the signal an entry stuck at `AwaitingRegistration` most needs. Three further things wake a lease, all of them updates: a readiness change, an adoption-label change, and the arrival or departure of `horizon.dev/watchdog-armed`. Without that filter, every kubelet status update would wake a reconcile that lists nodes uncached and writes status. Without the third signal, the filter would swallow the first arming and leave the `WatchdogArmed` condition from [0023](0023-observe-the-armed-watchdog-from-the-control-plane.md) waiting for a requeue.

The third signal watches for the annotation appearing and disappearing rather than for its value changing, because the agent rewrites it on every poll tick at a compile-time default of fifteen seconds. Waking on the value would put the lease reconciler on a fifteen-second-per-node treadmill, which is twice the rate of the poll the watch replaced. What the annotation's value carries, staleness, is a property of elapsed time rather than of an event, so the requeue remains the right instrument for it and the watch takes only the transitions.

`InstancePhaseJoined` stays sticky, and deliberately. It records that a node was ready once rather than that it is ready now, which is why the count above cannot be derived from it. Making it mean is-ready-now was tried and rejected on measured grounds: `stallReason` reads `Created` plus fifteen minutes since the provider created the instance as a registration timeout, so downgrading the phase when a kubelet restarts makes `retireStalledInstances` destroy a healthy server sixteen minutes into its lease and report `RegistrationTimeout` for a node that registered and ran. A regression test pins the current behaviour so the next attempt sees the consequence before making it.
