...
This record supersedes the interface decision in [0016](0016-cluster-agnostic-tool-with-provider-seam.md). The cluster-agnostic behaviour from that record stands, as does its rejection of a plugin framework: this widening is driven by a requirement that exists, not by an abstraction that might one day be useful. It also supersedes [0006](0006-cluster-api-operator-pivot.md), whose claim of owning no provider abstraction and no cloud credentials this record reverses; [0021](0021-node-side-dead-mans-switch-on-two-clocks.md) later puts a delete-capable token on every leased node.
