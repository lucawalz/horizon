...
The interface creates leases, extends running ones, releases them, and creates, replaces and deletes provider configurations, and does nothing else that writes: no Secret is created, read or rendered. The provider config form names the Secrets a configuration points at, which are made with `kubectl` in the namespace the controller runs in, and it reads the created configuration back so a wrong name or a missing key arrives as the reason the configuration is unready rather than as a guess made in the browser. Each config listed on the machines page opens a read-only detail view of what it is configured with: the image, the SSH keys and firewalls, the watchdog timings, the conditions the controller wrote, and the Secrets it references by name and key alone, since the reference is what tells a missing Secret apart from a misnamed one and no view resolves one. A published catalogue is tallied there by region rather than listed, because it runs to hundreds of entries and the machines page already lists what a region offers. The same list opens an edit form, which submits the whole spec so a field left out of it is cleared rather than kept, and is refused with `409` when the configuration moved between the read and the submission. Deleting one is refused while a capacity lease still names it, and the refusal names those leases, because horizon tears a lease down with the credentials that configuration resolves; a finalizer in the controller holds the object back whatever asks for the deletion, so `kubectl` is refused the same way. The lease create form asks for a requirement first, since that is what the controller resolves against the provider catalogue, and naming a machine type stays available as the escape hatch the custom resource already provides. The workload namespaces are entered one per field, with another added on request, and each is offered as suggestions where the signed-in identity may list namespaces and stays a plain text field where it may not, because that grant is an optional rule rather than part of the documented role, and a refusal is not worth reporting for a field that works without it. An entry left blank is dropped rather than submitted. Beside them the form carries an optional selector, which narrows the target set to the workloads inside those namespaces carrying the labels it names, and the mode the lease runs its workload in. Move repins each matched workload onto the leased nodes and puts it back at expiry; replicate writes to none of them and runs a lease-owned copy on the leased nodes instead, so the count of pods a copy runs is asked for only when replicate is chosen, in the same shape as the requirement-or-machine-type choice above it. The lease detail names that mode and the copies the lease holds, and it words the reasons replicate records against a workload it left uncopied, each naming move mode where move mode is the way to burst that workload. A minimum memory carries its unit as a visible choice with the resolved byte count beside it, because `4Gi` is 4,294,967,296 bytes and a machine advertising 4 GB has 4,000,000,000, so the binary suffix silently excludes it. Bounds are shown while typing and enforced by CEL on the apiserver, whose refusal is forwarded with its own status code and message rather than restated. Releasing a lease deletes the `CapacityLease`, which is how the controller is asked for a teardown through its finalizer; the confirmation names the two clocks rather than asking whether the operator is sure, and nothing in the browser destroys a machine itself. Extending one edits `spec.duration` and nothing else, because the controller derives the deadline from that duration on every pass. Only a lengthening is accepted: a shorter duration can move the deadline into the past, which spends the teardown budget before teardown starts and deletes the leased nodes without draining them, so releasing the lease is the path that returns capacity early. An extension measured against a duration the lease no longer carries is refused with `409` rather than applied, and a lease that has expired or is already being released is refused as well, since the controller re-derives no deadline for either. The lease detail carries the earliest lifetime backstop its machines latched, so the ceiling offered is one the controller will not clamp.
